...
export default function SideBar()
{
    ...
    return(
        <div className="flex flex-col items-center pt-5 h-screen shadow-xl gap-3" 
             style={{
                 position: windowSize < '768' ? 'fixed' : 'sticky' ,
                 left: windowSize < '768' ?  "-100%" : 0 ,
                 width: isOpen ? '290px' : 'fit-content'
                 }}>
              {
                LinksArray.map((item , key) => 
                  item.role.includes(User.role) &&
                  <NavLink key={key} to={item.path} className="flex justify-center items-center gap-2 bg-gray-300 rounded-[11px] text-[28px] font-[Roboto] text-purple-800 w-[90%] p-[10px]">
                      {item.icon}
